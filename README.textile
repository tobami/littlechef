...
Whenever you apply a recipe to a node, your cookbook and roles dirs are gzipped and uploaded to that node, to the @/tmp/chef-solo/@ directoy. A node.json file gets created on the fly and uploaded, and Chef Solo gets executed at the remote node, using node.json as the node configuration and the pre-installed solo.rb for Chef Solo configuration.
...
* @cook list_recipes@: Lists all recipes, with associated information
