...
@cookbooks/@: This will be your cookbooks repository. You should put this under version control. Note that it is not recommended to clone the whole opscode cookbooks repository because its 7 MB compressed would make synching cookbooks at the nodes a bit slow.
@roles/@: Define here your roles

h3. How it all works

Whenever you apply a recipe to a node, your cookbook dir gets gzipped and uploaded to that node. A node.json file gets created on the fly, and Chef Solo gets executed at the remote node, using node.json as the node configuration, the pre-installed solo.rb as Chef configuration, which specifies cookbooks and roles as local (@/tmp/chef-solo/cookbooks@).

The result is that you can play as often with your revipes and nodes as you want, without having to worry about repositories, central servers nor anything else. Once you are satisfied with a new feature in a cookbook, you can commit the @littlechef/cookbook/ directory@. LittleChef brings back sanity to cookbook development.
...
To be able to issue commands to remote nodes, you need to enter a user and a password with sudo rights. For that, create a file named @auth.py@ and enter:
@
[userinfo]
user     = yoursudouser
...
The haproxy is a modified opscode recipe so that it accepts an array of loadbalanced servers. You can edit the new configuration file at @nodes/MYNODE2.json@ and add some attributes. Make it look like this (ignore the littlechef custom fields):
